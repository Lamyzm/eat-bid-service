...
def test_목록_필수_column이_없으면_발견_전체가_SOURCE_CONTRACT로_닫힌다(
    missing: str,
) -> None:
    with pytest.raises(SourceContractError):
        parse_bid_list_page(
...
def test_schema_fingerprint는_dataset과_column_순서에_독립적이다() -> None:
    ...
    assert (
        parse_nexacro(first, require_ds_info=True).schema_fingerprint
        == parse_nexacro(second, require_ds_info=True).schema_fingerprint
    )
...
def test_parser가_nexacro_namespace을_요구한다() -> None:
    with pytest.raises(NexacroParseError):
        parse_nexacro(
            b"<Root><Dataset id='ds_info'><Rows><Row/></Rows></Dataset></Root>"
        )
...
@pytest.mark.parametrize(
    ("wire", "expected"),
    [
        # 2026-09-17 실측: 기관이 유의사항 칸에 꺾쇠를 그대로 입력했고 소스가 이스케이프하지 않았다.
        ("<연락처> 행정실 051-979-6115", "<연락처> 행정실 051-979-6115"),
        ("<b>강조</b>", "<b>강조</b>"),
        ("정상 &lt;표기&gt;", "정상 <표기>"),
        ("꺾쇠 없음", "꺾쇠 없음"),
        # 꺾쇠와 ampersand가 같은 본문에 함께 와도 둘 다 복구된다.
        ("<연락처>&급식실", "<연락처>&급식실"),
    ],
)
def test_이스케이프되지_않은_꺾쇠를_복구해_해석한다(wire: str, expected: str) -> None:
    parsed = parse_nexacro(_nexacro(wire), require_ds_info=True)

    assert parsed.datasets["ds_info"][0]["DLVRY_PLACE"] == expected


def test_꺾쇠_복구는_Col_밖의_구조_손상을_덮지_않는다() -> None:
    """왜: 복구 범위는 Col 본문뿐이다. Dataset·Row 골격이 깨진 payload까지 살려내면 관측과 해석을
    나눈 경계가 무너지고 우리가 만든 구조를 사실처럼 저장하게 된다(AGENTS 3)."""
    broken = (
        b'<?xml version="1.0" encoding="UTF-8"?>'
        b'<Root xmlns="http://www.nexacroplatform.com/platform/dataset">'
        b'<Dataset id="ds_info"><Rows><Row><Col id="A">1</Col>'
        b"</Rows></Dataset></Root>"
    )

    with pytest.raises(NexacroParseError):
        parse_nexacro(broken, require_ds_info=True)


def test_꺾쇠_복구도_DTD와_entity_공격을_계속_막는다() -> None:
    attack = (
        b'<?xml version="1.0"?><!DOCTYPE Root [<!ENTITY x "boom">]>'
        b'<Root xmlns="http://www.nexacroplatform.com/platform/dataset">'
        b'<Dataset id="ds_info"><Rows><Row><Col id="A"><b>&x;</b></Col></Row></Rows>'
        b"</Dataset></Root>"
    )
