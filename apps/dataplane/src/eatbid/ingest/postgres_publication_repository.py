...
class PsycopgPublicationRepository:
    ...
    def validate_run(
        ...
        *,
        ...
        with self._connection.transaction(), self._connection.cursor() as cursor:
            ...
            mode, status, parser_version, expected_count, _, _ = run
            if status in {"validated", "failed"}:
                return self._load_existing(cursor, run_id, publication_id)
            if status != "running":
    ...
    def _lock_attempt_outputs(
        ...
        *,
        ...
        cursor.execute(
            """
            select normalization_attempt_id, observation_id, parser_version, status
            from ingest.normalization_attempt
            where run_id = %s and observation_id = any(%s)
            order by normalization_attempt_id
            for update
            """,
    ...
    @staticmethod
    def _load_existing(
        cursor: psycopg.Cursor[Any], run_id: UUID, publication_id: UUID
    ) -> PublicationValidation:
        cursor.execute(
            """
            select publication_id, status, expected_count, normalized_count
        ...
        if publication[1] == "validated" and len(member_ids) != int(publication[3]):
            raise PublicationIntegrityError("publication member manifest is incomplete")
        if publication[1] == "failed" and member_ids:
            raise PublicationIntegrityError("failed publication cannot have members")
        return PublicationValidation(
            publication_id=publication_id,
            run_id=run_id,
            status=str(publication[1]),
