...
from eatbid.core.region_label_aliases import SIDO_LABEL_ALIASES
...
def official_label_paths(
    path: tuple[str, ...], aliases: Mapping[str, tuple[str, ...]]
) -> tuple[tuple[str, ...], ...]:
    """eaT 이름 경로를 행안부 정식명 경로 후보들로 바꾼다. 바꾸는 것은 **시도 토막 하나**뿐이다.

    원본 경로를 후보에 남겨 두는 이유는 소스가 이미 정식명을 보내는 경우가 있기 때문이다. 그때는
    별칭 없이도 같은 결론에 닿아야 하고, 별칭 표는 축약명에만 관여한다.
    """
    if not path:
        return ()
    return (path, *((official, *path[1:]) for official in aliases.get(path[0], ())))
...
def propose_region_mappings(
    *,
    ...
    sido_aliases: Mapping[str, tuple[str, ...]] = SIDO_LABEL_ALIASES,
    ...
    별칭 표가 여는 문은 시도 토막 하나이며, 그 대응은 관측이 아니라 사람의 승인이다. 그래서 시도
    자체를 가리키는 행은 `reviewed`로, 그 아래 시군구 행은 결정적 라벨 일치가 만든 `label_verified`로
    남는다. 두 상태를 나누지 않으면 "무엇을 사람이 보증했는가"가 표에서 사라진다(ADR 0035 결정 6).
    """
    ...
    resolved: dict[int, dict[int, int]] = {
        code_value_id: _resolve_targets(paths, target_index, sido_aliases)
        for code_value_id, paths in source_paths.items()
    }
    claimed = Counter(
        target_id for targets in resolved.values() for target_id in targets
    )
    ...
    for code_value_id in sorted(source_paths):
        if not source_paths[code_value_id]:
            without_label.append(code_value_id)
            continue
        targets = resolved[code_value_id]
        if len(targets) > 1:
        ...
        if not targets:
            ...
            continue
        target_id, depth = next(iter(targets.items()))
        ...
        candidates.append(
            RegionMappingCandidate(
                ...
                status=REVIEWED_STATUS if depth == 1 else LABEL_VERIFIED_STATUS,
            )
        )
...
def _resolve_targets(
    paths: set[tuple[str, ...]],
    target_index: Mapping[tuple[str, ...], int],
    aliases: Mapping[str, tuple[str, ...]],
) -> dict[int, int]:
    """관측 경로들이 닿는 `행안부 code_value_id → 경로 깊이`를 모은다.

    깊이를 함께 드는 이유는 상태가 그것으로 갈리기 때문이다. 깊이 1은 시도 자체이고 그 대응의 근거는
    승인된 별칭 표이며, 깊이 2 이상은 시군구 이름이 그 시도 안에서 유일하게 맞았다는 관측이다.
    """
    targets: dict[int, int] = {}
    for path in paths:
        for candidate in official_label_paths(path, aliases):
            target_id = target_index.get(candidate)
            if target_id is not None:
                targets[target_id] = len(candidate)
    return targets
