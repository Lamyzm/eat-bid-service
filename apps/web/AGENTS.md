...
- 의미 요소 선택, 접근 가능한 이름·역할·상태, 폼 연결, 표 머리글, 초점 이동과 숨김의 종류는
  [`eatbid-web-accessibility`](../../.agents/skills/eatbid-web-accessibility/SKILL.md) skill이 소유한다.
  기계 검사는 `.oxlintrc.json`의 `jsx-a11y` 규칙이며 severity를 낮추거나 `categories.correctness`를
  내려 우회하지 않는다.
