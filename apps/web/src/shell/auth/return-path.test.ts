...
describe('로그인 복귀 경로 검증', () => {
  ...
  test('redirect 대상은 실제 화면으로 좁히고 query는 보존한다', () => {
    ...
    expect(returnRoute('/setup?view=목록')).toBe('/setup?view=목록');
    ...
    expect(returnRoute('/setup?view=목록#상세')).toBe('/setup?view=목록');
  ...
  test('로그인 링크는 검증한 경로만 담는다', () => {
    ...
    expect(loginRouteWithReturn('//evil.example')).toBe('/login?next=%2Fsetup');
  });
