...
const analysisQueryKeys = {
  ...
  conditionOptions: (query: AnalysisConditionOptionsQueryInput) =>
...
export function createAnalysisQueries(request: ContractRequest) {
  return {
     ...
     */
    conditionOptions(input: AnalysisConditionOptionsQueryInput) {
      return queryOptions({
        queryKey: analysisQueryKeys.conditionOptions(input),
        queryFn: ({ signal }) => findAnalysisConditionOptionsWith(request, { ...input, signal })
      });
    }
  };
}
