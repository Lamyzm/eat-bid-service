...
describe('오늘 화면 search param', () => {
  test('필터 링크는 값이 없는 조건을 주소에 남기지 않는다', () => {
    ...
    expect(buildTodayRoute({ ...EMPTY_TODAY_SEARCH, item: '축산', closesWithinHours: 72 })).toBe('/today?item=축산&closesWithinHours=72');
  });
  ...
  test('조건 하나를 바꾸는 링크는 다른 조건을 지우지 않되 cursor는 되돌린다', () => {
    ...
    expect(buildTodayFilterRoute(search, { item: '축산' })).toBe('/today?region=41&item=축산&closesWithinHours=24');
