 ...
 *
 * 기둥은 xl 아래에서 본문 위로 내려온다. lg(1024)에서는 셸 탐색을 빼고 남는 폭이 736px이라 기둥
 * 244px을 떼면 표의 고정 열 합(492px)이 들어갈 자리가 없어 기관 열이 0으로 무너진다(2026-09-15 실측).
 */
export function TodayFrame({ header, rail, filters, list }: TodayFrameProps) {
  return (
    <div data-slot='today-screen' role='region' aria-labelledby='today-title' className='mx-auto grid w-full max-w-[1024px] min-w-0 gap-4 px-3 py-3 sm:px-4'>
      ...
      <div className='grid min-w-0 gap-x-6 gap-y-4 xl:grid-cols-[220px_minmax(0,780px)]'>
