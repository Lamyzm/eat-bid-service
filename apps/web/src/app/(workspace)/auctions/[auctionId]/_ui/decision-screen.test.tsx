...
describe('결정 화면', () => {
  ...
  test('금지 문구가 없다', () => {
    const markup = renderToStaticMarkup(<DecisionScreen decision={decision()} search={search} history={readyHistory} />);
  ...
  test('회차 이력이 있으면 흐름 차트·과거 회차 표·이 값이면 패널을 하나의 투찰률로 함께 그린다', () => {
    const screen = render(<DecisionScreen decision={decision()} search={search} history={readyHistory} />);
  ...
  test('구매기관이 정규화되지 않은 공고는 이유를 그대로 말한다', () => {
    const markup = renderToStaticMarkup(<DecisionScreen decision={decision()} search={search} history={{ state: 'no-organization' }} />);
