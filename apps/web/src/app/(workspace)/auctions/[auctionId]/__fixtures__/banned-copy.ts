...
export const BANNED_COPY_RULES: readonly BannedCopyRule[] = [
  ...
  // "이 값이면"·"지금 값을 그때 냈다면"은 사용자가 놓은 값의 과거 가정이라 통과한다.
  rule(
    '값 지목 지시',
    String.raw`(?:이|그|저|해당)\s*${VALUE_NOUN}\s*(?:으로|로|을|를|대로)\s*(?:그대로\s*)?(?:옮겨\s*)?${ENTRY_VERB}${DIRECTIVE_ENDING}`
  ),
  // 추천·권장·적정·최적은 판단 대행 어휘다. "추천값"·"추천합니다"는 잡고 "추천하지 않습니다"는 통과한다.
  rule(
    '추천 어휘',
    String.raw`(?:추천|권장)(?:합니다|해요|드립니다|드려요)|(?:추천|권장|적정|최적)(?:하는|할|된|되는)?\s*(?:가(?!능)|${VALUE_OR_ZONE_NOUN})`
  ),
  ...
  // "밀림"·"밀립니다"는 이 값이면 밀려난다는 예측이다. 과거 가정 "밀렸을 회차"는 통과한다.
  rule(
    '밀림 예측',
  ...
  rule(
    '자리 단정',
    String.raw`(?:여기|이\s*근처|그\s*근처|이\s*부근|이\s*정도|이쯤|여기쯤|이\s*자리|이\s*칸|이\s*값|그\s*값)\s*(?:가|이|는|은|면|이면|라면|정도면|쯤이면)\s*(?:됩니다|된다|돼요|충분|넉넉|안전|무난|유리|낙찰됩니다|낙찰될)`
  ),
  // 낙찰 확률·가능성은 예측가 없이는 나올 수 없는 수치다(AGENTS 7·8).
  rule('낙찰 확률', String.raw`낙찰\s*(?:확률|가능성)`)
];
