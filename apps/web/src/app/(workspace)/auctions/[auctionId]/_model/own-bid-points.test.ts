...
import { presentHistory, type HistoryRow } from './attempt-history';
import { buildOwnDisplayModel, ownObservedRange, ownSummaryText } from './own-bid-points';
...
describe('내 투찰 표시 모델', () => {
  test('같은 회차의 여러 제출과 같은 날의 여러 회차를 점 하나로 합치지 않는다', () => {
    const model = buildOwnDisplayModel(
      [row('8101', '9101'), row('8201', '9201')],
      [submitted('8101', '9101', [{}, { bidRate: { value: '101.975', unit: 'percentage-points' } }]), submitted('8201', '9201', [{}])]
    );
    expect(model.points).toHaveLength(3);
    expect(new Set(model.points.map((point) => point.submissionId)).size).toBe(3);
    expect(model.points.every((point) => point.time === model.points[0]!.time)).toBe(true);
    expect(model.summary).toEqual({ submitted: 2, submissions: 3, absent: 0, notObserved: 0, conflict: 0 });
  });
  ...
  test('제출 금액이 없으면 계산용 자리표시자로 채우지 않고 금액 미확인이다', () => {
    ...
    expect(model.points.map((point) => point.amountText)).toEqual([null, '43,120,180']);
    expect(model.points.some((point) => point.amountText?.includes('10,000,000,043,768'))).toBe(false);
  });
  ...
  test('행의 revision과 다른 결과는 점으로 그리지 않고 확인 불가로 센다', () => {
    ...
    expect(model.resultByAttempt.get('8101')).toBe('evidence-conflict');
  });
  ...
  test('명단에 없음·미관측·증거 불일치를 서로 다른 수로 세고 문구가 미참여를 말하지 않는다', () => {
    ...
    expect(model.points).toHaveLength(0);
    const text = ownSummaryText(model.summary);
  ...
  test('개찰일이 없는 행의 결과는 점으로 놓지 않지만 요약에는 센다', () => {
    const model = buildOwnDisplayModel([row('8101', '9101', { openedAt: null })], [submitted('8101', '9101', [{}])]);
    expect(model.points).toHaveLength(0);
    expect(model.summary.submitted).toBe(1);
  });
  ...
  test('점 범위는 값 사이에 여백을 두고 0 아래로 내려가지 않는다', () => {
    ...
    const range = ownObservedRange(model.points);
