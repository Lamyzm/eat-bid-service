...
describe('이 값이면 패널', () => {
  test('손잡이 값이 없으면 회차를 세지 않고 값 없음과 무엇을 하면 계산되는지만 말한다', () => {
    ...
    expect(screen.queryByText(/낙찰값이 이 값 이상/)).toBeNull();
    expect(screen.queryByText(/\d+회/)).toBeNull();
  });

  test('지난 회차 수와 낙찰값이 이 값 이상인 회차를 칸 스트립과 함께 보인다', () => {
    // 손잡이는 투찰률이라 비교 값도 투찰률 축이다. 예정가격을 모르는 5회차는 분모에서 빠진다.
    const screen = renderPanel('92.500');
    expect(screen.getByText('지난 15회 중 낙찰값이 이 값 이상')).toBeTruthy();
    expect(screen.getByText('10회')).toBeTruthy();
    expect(screen.getByText('투찰률 축끼리 견줌')).toBeTruthy();
  });

  test('그날 하한이 이 값보다 높은 회차가 없으면 그 행을 아예 만들지 않는다', () => {
    const screen = renderPanel('92.500');
    expect(screen.queryByText('그날 하한이 이 값보다 높음')).toBeNull();
  });

  test('그날 하한을 밑도는 값이면 하한이 그 값보다 높은 회차를 같은 분모와 함께 센다', () => {
    const screen = renderPanel('90.100');
    const row = screen.getByText('그날 하한이 이 값보다 높음').closest('div');
  ...
  test('회차가 칸으로 세기에 많으면 칸 대신 비율을 숫자로 함께 말한다', () => {
    ...
    const won = screen.getByText('지난 30회 중 낙찰값이 이 값 이상').closest('div');
