...
import { chooseCompletedState, closeIssue, openChildren, planIssueClose } from "./linear-close.mjs";
...
test("같은 issue를 가리키는 열린 pull request가 남아 있으면 닫지 않는다", () => {
  const plan = planIssueClose({
    merged: true,
    branch: "eat-176-analysis-conditions",
    apiKeyPresent: true,
    otherOpenBranches: ["eat-176-analysis-chart", "eat-99-mutex"],
  });
  assert.equal(plan.act, false);
  assert.match(plan.reason, /eat-176-analysis-chart/u);
});

test("다른 issue의 열린 pull request는 닫기를 막지 않는다", () => {
  const plan = planIssueClose({
    merged: true,
    branch: "eat-176-analysis-conditions",
    apiKeyPresent: true,
    otherOpenBranches: ["eat-99-mutex", "eat-80-runbook"],
  });
  assert.equal(plan.act, true);
});

test("자기 자신과 같은 branch는 남은 pull request로 세지 않는다", () => {
  const plan = planIssueClose({
    merged: true,
    branch: "eat-176-x",
    apiKeyPresent: true,
...
test("완료도 취소도 아닌 하위 issue만 열린 것으로 센다", () => {
  const open = openChildren([
    { identifier: "EAT-1", state: { type: "completed" } },
    { identifier: "EAT-2", state: { type: "canceled" } },
    { identifier: "EAT-3", state: { type: "backlog" } },
    { identifier: "EAT-4", state: { type: "started" } },
  ]);
  assert.deepEqual(open.map((c) => c.identifier), ["EAT-3", "EAT-4"]);
});

test("하위 issue가 열려 있는 부모는 닫지 않고 그 식별자를 남긴다", async () => {
  const { calls, fetchImpl } = fakeLinear([
    {
      data: {
        issue: {
          ...
          children: {
            nodes: [
              { identifier: "EAT-215", state: { type: "completed" } },
              { identifier: "EAT-218", state: { type: "backlog" } },
            ],
          },
          team: { id: "t1", states: { nodes: [{ id: "s2", name: "Done", type: "completed", position: 2 }] } },
        },
  ...
  const result = await closeIssue({ apiKey: "k", identifier: "EAT-176", fetchImpl });
  assert.equal(result.moved, false);
  assert.match(result.reason, /EAT-218/u);
  assert.equal(calls.length, 1, "변경 요청을 보내면 안 된다");
});

test("하위 issue가 전부 완료면 부모를 닫는다", async () => {
  const { calls, fetchImpl } = fakeLinear([
    {
      data: {
        issue: {
          ...
          identifier: "EAT-9",
          state: { id: "s1", name: "In Progress", type: "started" },
          children: { nodes: [{ identifier: "EAT-10", state: { type: "completed" } }] },
          team: { id: "t1", states: { nodes: [{ id: "s2", name: "Done", type: "completed", position: 2 }] } },
        },
      },
    },
  ...
  const result = await closeIssue({ apiKey: "k", identifier: "EAT-9", fetchImpl });
  assert.equal(result.moved, true);
