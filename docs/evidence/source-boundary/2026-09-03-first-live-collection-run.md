...
**그런데 `LAST_CHG_DT`는 투찰 도착을 반영하지 않는다.** 2026-09-06 재관측에서 `BID_CNT`가 오른 7건
모두 `LAST_CHG_DT`가 그대로였다
([목록이 남긴 네 질문의 실측](2026-09-06-list-open-questions.md) §3.1). 저장 정합성 요구와 열린
공고의 명단 추적이 이 한 필드에서 충돌하므로 재호출 조건을 `LAST_CHG_DT` 하나로 두지 말고
`BID_CNT` 변화나 열린 상태를 함께 봐야 한다.
...
## 12. 계약 모형을 어떻게 고칠지는 사용자 결정이 필요하다

11절의 답으로 계약 fingerprint를 응답 전체가 아니라 검토된 필수 부분집합으로 계산해 봤다. 새 단위
테스트 둘은 통과했지만 기존 명세 다섯이 깨졌다. 핵심은 이것이다.

`apps/dataplane/tests/integration/test_normalize_validate.py`
`test_unreviewed_source_column은_attempt를_보존하지만_publication은_차단한다`

`ds_info`에 `UNREVIEWED_FIELD`를 주입하면 attempt는 보존하되 발행은 막으라는 의도적 fail-closed
규칙이다. 소스가 조용히 바뀌는 것을 잡는 장치이며 규칙 3의 정신이다. 부분집합 방식은 그 장치를
무력화한다. 그래서 되돌렸다. 저장소는 이 변경 없이 깨끗하다.

### 두 요구가 모두 정당하다

* 공고 유형마다 선택적 dataset이 붙고 빠지는 것은 소스 변경이 아니라 정상 변이다. 이것을 위반으로
  보면 어떤 live 수집도 발행되지 않는다.
* 우리가 본 적 없는 column이 나타나는 것은 소스 변경이다. 이것을 통과시키면 소스가 바뀐 것을 모른 채
  발행한다.
...
* **필수 부분집합** — 파서가 의존하는 dataset과 column. 하나라도 빠지면 위반.
* **알려진 전체 집합** — 소스가 낼 수 있다고 확인한 dataset과 column 전부. 이 밖의 것이 관측되면 위반.

그러면 알려진 선택적 블록은 통과하고 미지의 column은 계속 막힌다. 지금 계약은 이 둘을 하나로
뭉쳐 놓아 어느 쪽도 제대로 못 한다.

### 이 모형이 요구하는 것

알려진 전체 집합을 실측으로 열거해야 한다. 하루치 85건에서 12가지 모양이 나왔지만 그것이 닫힌
집합인지 알 수 없다. 여러 창과 공고 유형에 걸쳐 표본을 넓혀야 하고, 그 결과가 검토된 계약과
`schema_fingerprint`의 구조를 바꾼다. `eat-v2` 범위의 결정이다.

사용자 판단이 필요한 지점은 하나다. **알려진 전체 집합을 어느 표본까지 측정하고 확정할 것인가.**
너무 좁으면 다시 전량 위반이 나고, 너무 넓히려면 전량 수집을 먼저 해야 하는 순환이 생긴다.
