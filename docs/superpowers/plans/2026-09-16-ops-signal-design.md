...
실행 결과(2026-09-16): R1은 v0.1.38(EAT-242)로 단독, R2·R3·R4는 v0.1.39(EAT-243·244·245)로 함께 나갔다. 아침
요약과 critical 재알림, 브라우저의 로그 링크 클릭은 실측 대기다. 다음은 R5(안쪽 서비스 상태 뷰), R6(밖의 상태
페이지, Cloudflare), R7(로그·대시보드를 Grafana Cloud 무료 티어로 — 생존을 위해, 메모리가 아니라)다.
...
## 8.1 순서 재정렬 (2026-09-16 저녁, 방향 점검)

R1~R4 뒤의 순서를 "보이는 것"에서 "돌아오는 것·재발 안 하는 것"으로 바꿨다. 근거는 셋이다. 복원을 한 번도 해 본
적이 없었고, 같은 부류의 계약 빈틈이 달마다 하나씩 나오며, 릴리스 때마다 사람이 auto-merge를 끄고 켰다.

| 순위 | 무엇 | 상태 |
|---|---|---|
| 1 | 복원 리허설 — RTO·RPO를 숫자로(EAT-250) | 2026-09-16 완료. 내려받기 453초, 복원 489초, 행 수 운영과 일치. runbook §4.3 |
| 2 | raw 5년치 계약 빈틈 일괄 조사(EAT-251) | 다음 |
| 3 | 릴리스 레인 toil 제거 — 사람이 auto-merge를 건드리지 않는 모양 | 다음 |
| 4 | SLO 셋 정의, 상태 뷰는 SLO 달성을 보여 줌(R5 재정의) | |
| 5 | API 실패율·지연을 server 로그에서 계산 | |
| 6 | 밖의 상태 페이지(Cloudflare Pages + 공개 R2 버킷; Supabase·Vercel 기각), Grafana Cloud 무료 티어(생존 목적) | |
