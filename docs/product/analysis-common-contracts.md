...
- 품목 코드 조건은 기관에만 적용한다. 비교군에는 품목 필드 자체가 없다. label·정규식으로 코드를 만들지 않는다.
  선택지의 unsupported는 아직 코드/매핑 지원 없음, unavailable은 이번 조회 불가,
  ready의 빈 배열은 지원 선택지 0개다. unsupported/unavailable에서도 기관 전체 품목 조회는 가능하다.
...
4. 공통 기간·하한율·방식·명단 범위·현재 attempt 제외를 적용하고 기관/비교지역 소속을 판정한다.
   기관 품목은 기관 집합만 좁힌다. 이 결과의 attempt/revision 쌍이 추이·분포·이력의 공통 관측 집합이다.
