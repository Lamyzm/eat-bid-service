---
id: PRODUCT-WORKSPACE-DOCK-IMPLEMENTATION
status: active
canonical_for: workspace-dock-implementation-and-verification-record
last_reviewed: 2026-09-14
review_trigger: workspace-dock-layout-or-lifecycle-change
---
...
분석은 기본 보기부터 전체 가용 **폭**을 쓴다. 차트 `크게 보기`의 기존 높이 배분 정책은
2026-09-14 EAT-214의 뷰포트 전체 점유로 대체한다. 현행 동작의 권위는
[`screen-system.md`](screen-system.md)의 전체보기 계약이다. 같은 캔버스를 유지하면서 탐색 공간까지
차트에 돌려주고, 공통 상세 패널은 전체보기 안에서 계속 사용할 수 있다.

일반 보기는 기존 문서 스크롤 하나와 sticky 공통 헤더·필터를 유지한다. 오른쪽 도구는 viewport에
남고 긴 명단만 패널 내부에서 스크롤한다. 중앙에 추가 세로 스크롤 wrapper를 만들지 않는다.
과거 회차 표의 집중 보기는 기존의 충분한 높이 조건에서 중앙 높이를 제한하고 이력 표만 내부 스크롤하며 문서
