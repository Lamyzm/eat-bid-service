...
선 하나) 그 아래에서는 본문 위에 눕는다. 옆에 서는 폭에서 기둥은 **화면에 붙어 자기 스크롤을 갖는다**(사용자 요청
2026-09-17) — 목록이 200행까지 서므로 같이 흘러가면 조건을 바꾸려고 맨 위까지 되돌아와야 한다. 눕는 폭에서는 붙이지
않는다. 조건이 본문의 모든 수보다 앞(왼쪽·위)에 있는 이유는 조건이 "이 화면의 모든 수가
...
걸지 않는다(둘은 다른 체계다, AGENTS 6). **시도 목록은 접지 않고 늘 펼쳐 둔다**(사용자 결정 2026-09-18) —
접어 두면 고른 시도 하나만 보이고 나머지 열여덟이 삼각형 뒤로 숨어 화면이 "이 지역밖에 없다"고 말하는
것처럼 읽힌다. 어느 지역으로 시작할지는 저장된 관심 지역이 정하고, 그것과 목록을 펼쳐 두는 것은 다른
문제다. 목록이 길어지는 것은 받아들인다 — 기둥은 `xl`에서 자기 스크롤을 가지므로 본문을 밀지 않는다.
시도 목록은 어휘 전체가 0건까지 서고, 시군구는 코드목록이 말한 상위
...
**목록의 행(날짜 머리 → 시각 묶음 → 카드).** 표가 아니다(EAT-260, 사용자 결정 2026-09-17 — "toss처럼 위계가 바로 서
있지 않다"). 묶음은 **두 단**이다. 위는 날짜 머리(`9월 18일 금 · 내일 · 144건`, 18/800)이고 그 아래가 시각 묶음
머리(`오전 9시 마감 · 3시간 뒤 · 2건`, 14/700 muted)이며 행에는 시각이 없다.

날짜를 한 단 위로 올린 이유는 반복이다. 시각 묶음마다 날짜를 적으면 하루치 서른 묶음이 전부 같은 글자로 시작해
날짜 경계가 시각 경계와 구분되지 않는다(사용자 지적 2026-09-17). 날짜는 바뀔 때 한 번만 말하고, 두 단은 크기와
굵기로 갈린다.

**날짜 머리는 스크롤을 따라 위에 붙는다.** 한 날에 백 행이 넘게 서므로 붙어 있지 않으면 몇 줄만 내려도 지금 보는
것이 언제 마감인지가 화면에서 사라진다. 셸 머리가 이미 위에 붙어 있어 그 높이(`--workspace-header-height`)만큼
내려 앉는다. 지난 시각의 묶음은 `지났어요`이고 흐리다(0건이 아니다). **묶음 머리에
상태색(red·amber)을 쓰지 않는다** — 오늘·내일이 본문, 그 뒤가 muted, 지난 묶음이 hint이며 급함은 색이 아니라
`3시간 뒤`·`내일` 글자와 마감 순 정렬이 말한다. 마감이 가까운 것은 결손도 사고도 아니라 관측된 일정이고, 오늘
마감이 0건인 날에는 첫 화면의 묶음 머리가 전부 `내일`이라 통째로 amber가 되어 §9.2가 경고한 대로 목록이
경고판이 됐다(design-judge 반려 2026-09-17). 행은 왼쪽 세 줄과 오른쪽
...
| 둘째 | 공고 제목(한 줄) · 공고번호(복사 손잡이) · `제한지역 미관측` · `재입찰`/`변경공고` | 제목은 상세 관측이라 없으면 `제목 미관측`이다. 공고번호는 eaT로 건너가는 손잡이다(EAT-248). 제한지역 미관측은 급한 일이 아니라 사실이라 색이 아니라 굵기다. 게시 종류도 같은 굵기이며 재입찰·변경공고에만 적는다 — 일반공고는 176,158/181,150이라 거기 붙는 표시는 신호가 아니라 배경이고, 미관측(eat-v5 전 해석)을 일반공고로 접지 않는다(EAT-262). 재입찰이면 셋째 줄의 `지난번`이 같은 판의 직전 시도일 수 있어 행을 다르게 읽는다. 지역은 행에 없다 — 기둥의 축이다. |
| 셋째 | `지금 N곳 · 지난번 N곳 (MM-DD) · 보통 N곳 N회 기준` — 세 절뿐이다 | eaT가 주지 않는 세 값이라 이 화면의 값어치가 여기다. 참여 0은 `아직 0곳`이고 굵지 않다. 단독입찰을 허용하지 않는 판이면 그 뒤에 `(혼자면 유찰)`을 붙인다 — 그 판의 0곳은 기회가 아니라 유찰 신호다(실측 181,150건 중 허용안함 180,703). 허용하는 판과 아직 못 본 판에는 붙이지 않는다(EAT-249). 못 센 판은 `참여 미관측`. 지난번은 같은 기관·같은 하한의 직전 개찰 회차 관측이고, 보통은 코호트 중앙값과 표본 수다(§8). |
...
**범위 밖(별도 issue).** 표본 부족 gate 필드 하나만 남았다. 프리셋에 저장되는 `지역 미상 포함`은
EAT-267이 구현했고, 전역 `--hint` 토큰 대비는 이 화면에서 실측으로 통과했다 — 4.5:1 미만은 `·` 구분
글리프뿐이고 그것은 `aria-hidden` 장식이라 대상이 아니다(2026-09-17 계산값 측정).
