...
| 둘째 | 지역(링크) · 공고번호(복사 손잡이) · `제한지역 미관측` | 공고번호는 eaT로 건너가는 손잡이다(EAT-248). 제한지역 미관측은 급한 일이 아니라 사실이라 색이 아니라 굵기다. 공고 제목은 계약에 아직 없다(후속). |
| 셋째 | `지금 N곳 · 지난번 N곳 (MM-DD) · 하한 아래 N · 보통 N곳 N회 기준` | eaT가 주지 않는 세 값이라 이 화면의 값어치가 여기다. 참여 0은 `아직 0곳`이고 굵지 않다 — 단독입찰 허용안함 공고의 0곳은 기회가 아니라 유찰 신호일 수 있다(EAT-249). 못 센 판은 `참여 미관측`. 지난번은 같은 기관·같은 하한의 직전 개찰 회차 관측이고, 보통은 코호트 중앙값과 표본 수다(§8). |
...
**범위 밖(별도 issue).** 행 둘째 줄의 공고 제목(목록 계약에 title이 없다), 표본 부족 gate 필드, 전역 `--hint` 토큰 대비.
