...
왼쪽 기둥이 소유한다. 기둥은 `2xl`(1536px)부터 셸 탐색 바로 옆에 **붙어** 서고(본문 카드 안에 띄우지 않는다, 구역 사이는
...
바꾸기`가 그 사실을 말한다(EAT-167). 저장된 조건 한 벌은 화면에서 `프리셋`이라 부른다(`조합`이 아니다).
...
28px에 32px 버튼, 검색 칸 44px. page-level 숫자 hero는 머리 문장의 `진행중` 건수 하나이고 같은 수를 축 줄·탭·제목에 다시
세우지 않는다(EAT-241·EAT-260). 기초금액 칸은 Enter로 제출하고 `적용` 버튼이 없다. 표는 본문 폭을 다 쓰고 달력은 672px에서 멈춘다 — 두 주치 격자는 넓어질수록 읽기 어렵다.
