...
**구조와 순서.** 위에서 아래로 제목·기준 시각 → 탭(진행중·오늘 열린·오늘 마감) → 조건(축) → 검색 한 칸 → 마감 달력 →
목록이다. 조건이 탭 아래인 이유는 조건이 "이 화면의 모든 수가 어떤 집합을 세는가"의 선언이고 그 선언은 탭이 고른 판
안에서 읽히기 때문이다(§6.4.1). `2xl`(1536px)부터 조건은 왼쪽 기둥으로 서고 그 아래에서는 표 위에 눕는다.
지역은 사업자 설정의 참가제한지역이 기본 게이트이고, 게이트 문장과 `전체 보기`·`지역 바꾸기`가 그 사실을 말한다(EAT-167).

**목록의 열(여덟 칸, 머리글 일곱).** 폭은 셀 값이 아니라 머리글 두 줄이 정하며, 부제는 그 열의 값이 무엇에서 온
값인지를 말한다. `xl`(1280)에서 여덟 칸이 다 서고, `lg`에서 지난번·보통이, `md`에서 품목·기초금액까지 접혀 기관 셀의
둘째 줄로 내려간다. 접힌 칸은 지워지지 않는다.

| 열 | 부제 | 왜 이 열인가 |
|---|---|---|
| 순번 | (머리글 숨김) | 마감 임박 순이 이미 정해진 목록이라 "몇 번째로 닫히나"다. |
| 마감 | 개찰 한 시간 뒤 | 시각만이다. 날짜는 묶음 머리가 한 번 말한다. 오늘·내일은 색, 그 뒤는 본문 색이다. |
| 기관 | — | 이름 아래 지역·공고번호(복사 손잡이)·제목. 폭이 남는 유일한 열이다. |
| 품목 | 저장된 라벨 | 원천 라벨 그대로다. 필터와 배지는 원자 코드 조인이고 라벨은 표시값이다(EAT-230). |
| 기초금액 | 저장된 값 | 통화를 동반한 정확한 값이다. 예정가격은 없다. |
| 참여 | — | 마지막 훑기의 명단 수다. 단독입찰 허용안함 공고의 0곳은 기회가 아니라 유찰 신호일 수 있어 뜻을 화면이 말하기 전까지 값만 싣는다(EAT-249). |
| 지난번 | 같은 하한 직전 | 같은 기관·같은 하한의 직전 개찰 회차 관측이다. |
| 보통 | 중앙값 · 표본 | 코호트 중앙값과 표본 수를 함께 적는다. 표본이 얇아도 지우지 않고 수를 보인다(§8). |

하한율은 열이 아니라 요약(`floorSpread`)이고, 낙찰 투찰률은 열이 아니다 — 이 목록의 눈금(사정률)과 다르고 행마다
서면 앵커링이라 결정 화면으로 보낸다(PDR-0004).
...
- 표본 부족: 보통 열은 표본 수를 함께 적을 뿐 문턱으로 값을 감추지 않는다.
- 검색 중: 검색이 지금 조건 안에서 세고 있음을 문장이 말한다(`지금 조건 안에서 "김해" · 84건`).

**밀도.** 본문 13px, 숫자는 tabular-nums다. 표 행 44px, 마감일 묶음 머리 36px, 달력 칸 48px(두 줄), 조건 기둥의 줄
28px에 32px 버튼, 검색 칸 44px. page-level 숫자 hero는 `진행중` 탭의 건수 하나이고 같은 수를 축 줄이나 제목에 다시
세우지 않는다(EAT-241). 표는 본문 폭을 다 쓰고 달력은 672px에서 멈춘다 — 두 주치 격자는 넓어질수록 읽기 어렵다.
...
**범위 밖(별도 issue).** 목록 행 재설계(`lastRound` 셋째 줄·eaT 손잡이·표본 부족 gate 필드)와 전역 `--hint` 토큰 대비.
