...
| rail 이 값이면 | 손잡이 값을 기준으로 낙찰값이 그 값 이상이었던 회차, 그날 하한이 그 값보다 높았던 회차, 보통 참여, 낙찰값 위 0.1 안 곳수(주어는 과거 회차다 — `decision-support.md` §9 금지 목록, EAT-236) | `mart.org_round_summary` + `core.bid_submission` | 회차 | — | 위와 같음 | 요청 시 N ≤ 200행 계산 |
...
- `win_rate_distribution_monthly`: (scope, region_code, item_code, floor_rate, award_method, month, bin) → count. 기간 조회는 월 행 합산. 지역 코드 체계는 열이 아니라 build가 기록한다(`mart.build.region_scheme`). 지금 build는 eaT 공고지역이고 행안부 코드를 적재한 뒤에는 새 `calc_version`의 새 build가 그 체계로 만든다(규칙 6). **품목 축은 비어 있다** — 빌더가 `item_code_value_id`에 항상 null을 넣고 이 표에는 `item_label` 열 자체가 없다. 품목 `CodeScheme`이 없어 관측 라벨을 코드로 승격시킬 수 없기 때문이며(AGENTS 2), 그래서 `findWinRateDistribution`은 `itemCode`를 받지 않고 응답 meta에 `item: null`을 명시적으로 실어 "품목으로 좁히지 않은 표본"임을 화면이 말하게 한다. 조회 SQL도 `item_code_value_id is null`을 명시적으로 써 EAT-66이 품목 행을 넣는 순간 같은 회차를 두 번 세지 않게 한다. 실측 부하는 [`docs/evidence/mart/2026-09-06-win-rate-distribution-scan.md`](../../evidence/mart/2026-09-06-win-rate-distribution-scan.md)에 있다(38,050행 표에서 읽은 행 37, 0.175 ms, 새 index 없음).
...
이 절의 항목은 답이 정해지는 순간 [제품 기획 결정 기록](../decisions/README.md)으로 옮겨 적고 여기에는 PDR 링크를 남긴다.
...
이 뒤집힘은 [PDR-0002](../decisions/0002-day-floor-is-derived.md)로 승격됐다. 결정의 권위는 그쪽이며 이 절은 관측 기록으로 남는다.
