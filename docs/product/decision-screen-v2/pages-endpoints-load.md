...
| 항목 | 결정 | 이유 |
| -- | -- | -- |
| 캐시 키 | 응답 헤더 `ETag = martRelease + 요청 파라미터 해시`, web은 `use cache` + `cacheTag('mart:<release>')` | 같은 mart release면 DB를 다시 읽지 않는다. release 전환 시 태그 하나로 무효화 |
...
| 흐름(회차별 낙찰률 선) + 과거 회차 표 | `listOrganizationAuctionAttempts` GET `/api/v1/organizations/{organizationId}/auction-attempts` | path org id; query: itemCode?, cursor?, limit(기본 12, ≤ 200), until?(공고 시각) | attempts[{attemptId, auctionId, announcedAt, openedAt, floorRate, baseAmount, winRate, secondRate, dayFloorRate, listCount, invalidCount, winnerSupplierId, supersedesAttemptId}], nextCursor | `mart.org_round_summary` (org_id, announced_at desc) 인덱스, 기관당 5년 ≤ 200행 | 0.3 req/s × 12행. 무시 가능. 흐름은 첫 페이지 60행을 한 번에 받아 같은 응답으로 표와 차트를 그린다(요청 2개 금지) | RSC → 흐름 inline SVG, 표 TanStack Table headless. 페이지 cursor는 URL. 태그 `org:<id>` + `mart:<release>` |
