...
(`canonical_record_object`의 `exclude_unset`, [ADR 0038](../adr/0038-additive-ingestion-fields-and-parser-version.md)).
...
#### wire 비율 계약 여섯이 나뉘어 있는 이유

`packages/contracts/src/values/rate.ts`의 여섯은 표현이 겹쳐도 범위와 소비자가 다르다. 하나로
