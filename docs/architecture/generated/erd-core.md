<!-- 생성물이다. 직접 편집하지 않고 `pnpm architecture:erd:write`로 다시 만든다. 원천: packages/db/drizzle/20260916163445_solo_bid_method/snapshot.json -->
# `core` 스키마 ERD

Drizzle 마이그레이션 `20260916163445_solo_bid_method`의 snapshot에서 만든 표·컬럼·외래키 그림이다. 표 18개.
