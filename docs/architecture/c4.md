...
```mermaid
flowchart LR
    source[Pydantic source contract] --> db[(PostgreSQL exact storage)]
    db --> adapter[Drizzle adapter]
    adapter --> domain[packages/domain\nsemantic values]
    domain --> contract[packages/contracts\nZod wire schemas/codecs]
...
`packages/contracts`는 JSON으로 직렬화 가능한 Zod wire schema를 소유한다. 두 package는 DB row나
Pydantic source shape를 권위로 재사용하지 않으며 adapter와 cross-language fixture가 의미 보존을
검증한다. 상세 규칙은 [time-and-value-contracts.md](time-and-value-contracts.md)를 따른다.
