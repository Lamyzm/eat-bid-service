...
이 Level 3 흐름은 frontend 화면/상태 구조를 자동 전환하거나 누락 좌표를 이름 lookup으로 채우지 않는다.
web은 공동 제품 설계 후 public wire parser 소비자로 전환하고, 좌표 enrichment는 CRS/provenance를 가진
별도 Argo workflow/backfill로 다룬다.
