...
| Pydantic `TypeAdapter` | Validate each Nexacro `Dataset/Row/Col` source row as `dict[str, StrictStr]` before normalization; `NormalizedAuction` is the strict normalized authority | [TypeAdapter API](https://docs.pydantic.dev/latest/concepts/type_adapter/) | Adopted | Task 8 exercises a real non-model source fragment and then validates the normalized model once; review on Pydantic major change or source-fragment drift. |
