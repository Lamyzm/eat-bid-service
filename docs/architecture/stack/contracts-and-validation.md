...
| Hypothesis | Property-test leading-zero codes, deterministic normalization/gzip/replay, and count/completeness gates | [Hypothesis documentation](https://hypothesis.readthedocs.io/) | Deferred | Add in the owning data-foundation tasks with those invariants, not as an unused dependency today. |
...
| Testcontainers | Validate migrations and contract integration against disposable PostgreSQL | [Testcontainers Node](https://node.testcontainers.org/) | Required before production | Trigger before a production migration pipeline or cross-runtime integration release. |
