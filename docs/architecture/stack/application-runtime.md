...
| Cache Components | `cacheComponents: true`, canonical route는 Suspense 격리, legacy `/dashboard/**`는 `instant = false` | [Cache Components](https://nextjs.org/docs/app/getting-started/partial-prerendering), [self-hosting](https://nextjs.org/docs/app/guides/self-hosting), [ADR 0028](../../adr/0028-cache-components-and-self-hosted-cache.md) | 채택, 조건부 | replica 1에서만 in-memory cache를 허용한다. replica 증가 전 공유 cache handler, edge runtime 금지, `use cache`는 resource server read 함수에만. 조건 변경은 새 ADR이 필요하다. |
