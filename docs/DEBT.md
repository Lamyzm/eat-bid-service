...
# 27. 2026-08-29 재파싱 사고에서 나온 항목 셋

게이트 A 측정 중에 새로 선 것들. **판정은 리더가 한다 — 여기는 근거와 처방 후보다.**

## 27-1. `db-migrate`가 로더 트랜잭션에 막혀 배포와 폴링을 동시에 세운다

**실측(리더, 2026-08-29 적재 중):**
```
PID 24954  db-migrate  Lock/relation      22600 이 막고 있음, 35분52초
PID 28141  폴링 잡      Lock/relation      delete from open_auctions, 12분57초
PID 22600  로더         Client/ClientRead  막는 것 없음
```

**기전은 락 하나가 아니라 FIFO 락 큐다.** 로더는 `load_postgres.py:216`(`delete from school_roster`)부터 `:578`(`pg.commit()`)까지 **단일 트랜잭션**이다. `db-migrate`가 그 안에서 잡힌 락을 기다리기 시작하면, **뒤에 오는 요청은 현재 보유자와 호환되더라도 대기 중인 DDL 뒤에 줄을 선다.** 그래서 폴링 잡까지 막혔다.

**⚠ 로더의 단일 트랜잭션은 건드리지 마라.** delete+rebuild를 한 트랜잭션에 넣은 건 **롤백을 보장하려고** 그렇게 한 것이고, 오늘 로더가 죽었을 때 그게 우리를 구했다. **안전과 가용성이 맞바꿈된 자리다.**

**고칠 곳은 마이그레이션 쪽이다.** `app.yaml:184`:
```sh
psql -v ON_ERROR_STOP=1 postgres://… -f /schema/schema.sql
```
**`lock_timeout`도 `statement_timeout`도 없다.** 그래서 무한정 기다린다.

| 처방 후보 | 비용 | 효과 |
|---|---|---|
| **`PGOPTIONS='-c lock_timeout=5s'`** | 한 줄 | **35분 조용한 적체 → 빠르고 시끄러운 실패.** ArgoCD가 sync 실패로 표시하고 사람이 본다. `backoffLimit: 2`로 3회 재시도 |
| `CREATE INDEX CONCURRENTLY` | 중간 | 락을 안 잡는다. 다만 트랜잭션 블록 안에서 못 돌고, 실패 시 invalid 인덱스가 남는다 |
| 스키마가 실제로 바뀔 때만 실행 | 큼 | ArgoCD 훅은 **매 sync마다** 돈다. ConfigMap이 `disableNameSuffixHash: true`라 내용 변화를 이름으로 못 가른다 |

**내 권고: 첫 번째.** 손해는 "적재 중 배포가 실패하고 나중에 다시 sync" 이고, **지금 손해는 "35분간 아무도 모른 채 배포·폴링이 멈춤"** 이다. 후자가 나쁘다 — 이 문서 전체가 그렇게 말한다.

**⚠ 그리고 `CREATE INDEX IF NOT EXISTS`도 락을 잡는다.** "아무것도 안 바뀌었으니 무해하다"가 아니다. **no-op sync도 막힌다.**

## 27-2. `order by filename desc`는 파티션이 갈리면 쓰기 순서가 아니다

`lake.py:72` 주석이 이렇게 적어놨다:
> *"파일명에 단조 증가하는 타임스탬프를 앞세워, `connect()`의 '최신 파일이 이긴다' 중복 제거가 **filename 문자열 정렬로도 실제 쓰기 순서를 반영**하게 한다."*

**그 문장은 같은 파티션 안에서만 참이다.** `filename`은 전체 경로다:
```
…/auctions/year=2024/month=06/part-01787…-<uuid>.parquet
              ^^^^^^^^^^^^^^^^ 여기가 part- 보다 앞에 온다
```
**문자열 desc 정렬은 `year=`를 먼저 본다.** 같은 `bid_id`가 두 파티션에 걸치면 **ns와 무관하게 큰 연·월 쪽이 이긴다.**

**언제 갈리나:** 파티션은 `_partition(announced_at)`이 정한다(`lake.py:39-43`). **재파싱이 어떤 회차의 `announced_at`을 바꾸면 그 회차의 새 파일이 다른 디렉터리에 떨어지고, 옛 파일은 원래 자리에 남는다.** 그러면 둘 중 경로가 큰 쪽이 이기는데, 그게 옛 것일 수 있다.

**이번엔 실현되지 않았다** — 옛 파일이 이긴 회차 0. **다만 우리가 "단조 파일명이니 최신이 이긴다"를 여러 번 근거로 썼고, 그 문장은 조건부다.**

**처방 후보:** 정렬 키를 경로가 아니라 파일명에서 뽑은 ns로 바꾼다 —
`order by regexp_extract(filename, 'part-(\d+)-', 1) desc`. 값이 20자리 제로패딩이라 문자열 정렬이 곧 수치 정렬이다(실측: 전 파일 10,906개에서 파싱 실패 0). **비용은 한 줄이고, 파티션이 갈려도 참이 된다.**

⚠ **다만 이 처방은 파일명 형식에 의존을 하나 더 만든다.** 지금은 `write_batch`만 그 형식을 만들지만, 손으로 넣은 파일이 하나라도 있으면 `regexp_extract`가 빈 문자열을 주고 **그 행이 정렬 맨 뒤로 간다(= 항상 진다).** 넣으려면 **못 뽑은 파일 수를 세어 0이 아니면 말해야 한다.**

### 🔴 기한 — **날짜가 아니라 조건이다**

**리더 판정(2026-08-29): 지금 안 고친다. P1이 들어간 뒤 그 위에 얹는다.** 두 개의 파일명 의존을 따로 관리하지 않기 위해서다.

**그런데 "나중에"는 이 프로젝트가 여섯 번 잊은 그 단어다.** 그래서 날짜 대신 **실현 조건**을 기한으로 못박는다:

> ## ⚠ **다음 재파싱 전에 닫는다.**
> **이 결함은 재파싱이 `announced_at`을 바꾸는 날 실현된다.** 회차의 파티션(`year=`/`month=`)이 옮겨지면, 옛 파일과 새 파일이 다른 디렉터리에 있게 되고 **경로 문자열 정렬이 ns를 이긴다.** 그날 옛 행이 이기고, **아무 예외도 안 난다** — 그냥 옛 값이 나온다.
>
> **재파싱을 계획하는 사람이 이 줄을 먼저 읽어야 한다.** `scripts/reparse.py`를 여는 것이 그 시점이다.

**날짜(9/12)보다 이게 강한 이유:** 9/12에는 아무 일도 안 일어나므로 그날 이걸 볼 이유가 없다. **재파싱하는 날에는 반드시 `reparse.py`를 연다.** [[failing-loudly]] F1-c와 같은 원리다 — **잊히지 않는다 = 그 일을 하려면 반드시 지나가는 자리에 있다.**

## 27-3. `select *` 하나면 `auctions`가 깨진다 — 이동 후에도 남는다

`.claude/rules/failing-loudly.md` **F1-b·F1-c**로 옮겨 적었다. 요지만:

- 중첩 구조체 필드 수가 다르면 **순서와 무관하게 예외**(실측: 4필드 먼저·5필드 먼저 둘 다 FAIL). **운이 아니라 결정적이다**
- **조용한 경로는 최상위 컬럼이다.** `union_by_name`이 없는 컬럼을 NULL로 채운다 — 그리고 **우리는 이미 당했다**(`sigungu_code` 추가 후 A-1이 5.76%)
- 감지는 **읽는 자리가 아니라 쓰는 자리**에서(`connect()` 3.7s vs `parquet_schema()` 전량 11.6s). `write_batch`가 O(1)로 지문을 남기고, 게이트는 이미 있는 `drift.record()` → `load_postgres.py:201`을 쓴다
- 삭제·개명은 배치가 못 본다 → `tools/lake/schema_census.py`를 사람이 부른다
