...
- 운영 DB의 매핑률은 아직 0이다. 2026-09-07(EAT-75)까지 정규화 수집 계약에 `PDLC_NM`을 실을 자리가
  ...
  라벨을 `core.code_label_observation`까지 옮긴다([ADR 0038](../adr/0038-additive-ingestion-fields-and-parser-version.md)).
...
- 라벨 경로는 2026-09-07(EAT-75)에 코드로 닫혔다. `auction.v2` 계약의 가산 optional 필드
  ...
  ([ADR 0038](../adr/0038-additive-ingestion-fields-and-parser-version.md)).
