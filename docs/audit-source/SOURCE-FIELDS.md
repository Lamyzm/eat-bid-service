...
- 학교 위치축에는 **상세 응답에 이름 필드가 없다.** `SIDO_NM`·`SIGUNGU_NM`은 `ds_info`에 아예 오지 않는다.
  **그러나 이름 자체는 eaT가 준다 — 다른 호출에 온다.** 목록 화면의 시도·시군구 콤보가 부르는 공통
  코드 서비스(`/cmmn/code/selectCodeListEhcache.do`, 그룹 `SC066`·`SC067`)가 코드마다 `CMNS_CD_NM`을
  주고 로그인이 필요 없다. 2026-09-16 실측에서 우리가 관측한 시도 15·시군구 201 코드 전부가 그 목록에
  있었고 미매칭이 0이다(EAT-187). 그래서 "소스가 학교 위치 지역의 이름을 안 준다"는 말은 **상세 응답에
  한해서만** 맞다.
...
1. 코드북 그룹과 우리 필드의 대응은 **이름 유사성 기준 추정**이며 값까지 대조한 그룹은 일부뿐이다(§11).
   값까지 대조해 확정한 그룹은 여섯이다 — `SC066`(공고지역 시도) · `SC067`(공고지역 시군구) ·
   `EP049`(공고 진행상태) · `BC016`(기관 유형)은 2026-09-16, `EP051`(예정가격 산정 방식 `PLNPRC_TYPE_CD`,
   001 단일·002 복수) · `EP111`(낙찰자 결정 방법 `SUCBID_DCSN_MTH_CD`, 001~019 중 17종 활성)은 2026-09-17에
   코드목록 전체 조회(그룹 144개)에서 값을 대조했다(EAT-200). 여섯은
   `apps/dataplane/src/eatbid/source/eat/code_schemes.py`의 `EAT_CODE_LIST_GROUPS`가 체계와 짝지어
   갖는다(EAT-187). `EP111`의 이름은 `예정가격의 []%이상 입찰가 중 최저가 낙찰`처럼 자리표시를 품은 코드북
   문장이고, 상세의 `SUCBD_DECISION_MTHD_NM`은 그 자리에 하한율을 채운 렌더링이라 이름이 아니다(T16). 우리
   코퍼스의 방식 8종(003·007·008·009·010·013·014·017)은 전부 `EP111`에 있다.
2. **전체 그룹 목록을 모른다.** 그룹 id를 알아야만 받을 수 있고, 아는 id는 폼에서 긁은 것뿐이다(§10.7).
   **`ITM_VL2`의 뜻은 그룹마다 다르다.** `SC067`에서는 부모 시도 코드지만 `BC016`에서는 BC016 코드가
   아닌 묶음 번호다. 한 이름을 그룹에 상관없이 같게 읽으면 `010 학교`의 상위가 `002 광역지자체`가 된다.
