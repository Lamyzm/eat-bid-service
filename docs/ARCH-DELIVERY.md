...
### 7-6. 검증 — "다 왔나"에 답하는 한 줄

```bash
find data/raw -type f | wc -l      # 219,487 이어야 한다 (2026-08-28 기준, 백필로 증가)
sha256sum -c lake-YYYYMMDD.sha256
```

**파일 개수 대조가 스탬프 대조의 데이터판이다** — C7이 이미지에 대해 한 것을 아카이브에 대해 한다. "복사 명령이 성공했다"와 "다 왔다"는 다르다.

### 7-7. 🔴 지금 당장 할 것

**사본이 하나도 없다.** VM이든 새 PC든 이전 계획과 **무관하게**, 오늘:

```bash
tar -cf - data/ | zstd -3 -o lake-20260828.tar.zst && sha256sum lake-20260828.tar.zst > lake-20260828.sha256
```

**한 시간이면 끝나고, 이 문서 전체에서 비용 대비 손실 방어가 가장 큰 항목이다.** R2 설정이 아직이면 일단 다른 물리 디스크에라도 둔다.
