...
**⚠ 219,487개를 개별 업로드하지 마라.** 파일 하나당 Class A 작업 하나이고, 작은 파일 22만 개는 느리며, 무엇보다 **"다 왔나"를 검증하기 어렵다.**

#### 구현 — `tools/backup/r2_archive.py` ✅

**단일 tar 가 아니라 샤드 단위 tar.xz 다.** 초안은 전체를 객체 하나로 묶으려 했는데, 그러면 **한 파일이 바뀌어도 2.2GB 를 다시 올린다.** raw 는 이미 256개 샤드로 갈려 있으니(`archive.py: _shard()` — bid_no 의 MD5 앞 2자) 그 경계를 그대로 쓴다.

```
R2:  raw/internal/00.tar.xz  …  raw/internal/ff.tar.xz   (샤드당 객체 1개)
     raw/internal-list/…
     manifest.json                                        ← 진실. 집이 타도 여기 남는다
```

| 결정 | 근거 |
|---|---|
| **샤드 단위** | 증분이 된다. 바뀐 샤드만 다시 올린다. Class A 는 실행당 수십 회 수준 |
| **xz preset 3** | 리더 실측 — `-9e` 는 33% 더 줄자고 샤드당 52초 × 256 = **3.7시간**이다. `-3` 이 1초에 절감의 78% |
| **stdlib `lzma`** | `xz` 바이너리·`rclone` 을 안 부른다. 의존은 `boto3`(이미 설치됨) 하나 |
| **지문으로 증분 판정** | 내용을 안 읽고 `(이름,크기,mtime)` sha256. 재수집은 같은 이름에 덮어쓰므로 **개수만으로는 변경을 놓친다** |
| **일부 실패 시 매니페스트 미갱신** | 반쯤 올라간 걸 다음 실행이 "완료"로 읽으면 그 구멍이 영구가 된다 |

#### 🔴 여기서 한 번 틀렸다 — `.gz` 를 풀어서 묶어야 한다

리더 실측표에 안 적힌 전제가 있었고, 그걸 놓쳐서 **절감 0%** 를 받았다.

| 방식 | 크기 | 절감 |
|---|---|---|
| 개별 `.gz` 합계 (현재) | 6,501 KB | — |
| **`.gz` 그대로** tar+xz-3 | 6,449 KB | **1%** ← 처음 구현 |
| **풀어서** tar+xz-3 | **3,064 KB** | **53%** ← 리더 실측 3,133 KB 와 일치 |

**이미 압축된 것을 다시 압축하면 얻을 게 없다.** 리더가 쓴 *"창이 32KB 라 파일 경계를 못 넘는다"* 가 바로 그 말이다 — **경계를 없애려면 먼저 풀어야** xz 가 파일들에 걸친 반복 태그 구조를 본다.

> **⇒ 아카이브 안에는 평문 `.xml` 이 들어간다. 그래서 복원은 `tar -xf` 하나가 아니다** — 다시 gzip 해야 크롤러가 아는 `{bid_no}.xml.gz` 가 된다. 그냥 풀면 크롤러가 파일을 못 찾아 **전량 재수집**한다. `--restore-shard` 가 그 재gzip 까지 한다.

#### 🔴 로컬 raw 는 건드리지 않는다

크롤러가 `{bid_no}.xml.gz` **존재**로 재수집을 건너뛴다. 로컬을 샤드로 묶으면 그 판단이 깨진다.
**로컬 = 임의접근(크롤러가 읽는다) · R2 = 냉동보관(사람이 복원할 때만).** 스크립트는 raw 를 **읽기만** 한다.

#### 전망과 비용

실측 raw 1,776 MB(실제 바이트) × 0.47 ≈ **835 MB**. 무료 10GB 의 8%다. 넘어도 $0.015/GB·월이라 30GB 여도 월 $0.30.

#### 자격증명 — `tools/backup/r2_setup.py`

```
python tools/backup/r2_setup.py          # 물어보고 .env.r2 에 넣는다
python tools/backup/r2_setup.py --check  # 연결만 확인
```

- **`getpass` 로 받는다.** 붙여넣을 자리가 없으면 사람은 결국 명령줄에 적고, 그러면 셸 히스토리·화면·대화 기록에 남는다. 한 번 새면 회수가 안 된다
- **저장 전에 연결을 먼저 확인한다.** 틀린 값을 저장하면 다음 실행이 조용히 실패한다
- **`git check-ignore` 로 실제로 막히는지 확인한다.** "gitignore 에 적었다"는 "막힌다"가 아니다
- ⚠ `.gitignore` 가 `.env`·`.env.local` 만 막고 있었다 → **`.env.r2` 는 그대로 커밋됐을 것이다.** `.env*`·`*.token`·`credentials.json` 으로 넓혔다
- **토큰은 Delete 없는 `Object Read & Write`.** 스크립트가 탐침 객체를 지워보고, **지워지면 경고한다** — 삭제 권한이 붙은 토큰은 스크립트가 미쳤을 때 R2 사본까지 지워 **상관된 백업**이 된다
...
### 7-7. 정기화 — `tools/backup/snapshot.py` ✅

⑤-e("간격은 날마다 커진다") 때문에 1회 스냅샷은 감가한다. 손으로 한 것을 그대로 스크립트로 옮겼다.

```
python tools/backup/snapshot.py [--dest H:\backup\eatbid] [--keep 30]
```

**구조를 이렇게 가른 게 이 스크립트의 유일한 설계 판단이다:**

| | 방식 | 왜 |
|---|---|---|
| `current/` | **한 곳에 증분** | raw·parquet 은 append-only 라 **옛 판이 현재 판의 부분집합**이다. 세대를 남길 이유가 없다 |
| `state/<ts>/` | **세대 (기본 30)** | sqlite 는 제자리 변형이다. **원본이 손상되면 증분 갱신이 그 손상을 사본에 덮어쓴다.** 세대가 유일한 방어다. 12MB/세대라 30세대도 360MB |

그 외 규율:
- **물리 디스크를 매 실행 확인하고 같으면 멈춘다.** 같은 디스크의 다른 파티션은 백업이 아니다
- **`/MIR`·`/PURGE` 를 안 쓴다.** append-only 데이터에서 원본의 파일 소실은 의도가 아니라 손상일 가능성이 높다 — **그 삭제를 사본에 전파하면 안 된다**
- robocopy 종료코드 **8 미만이 정상**이다. `|| true` 로 삼키지 않고 8 이상이면 exit 1
- `last-success.json` 은 **성공했을 때만** 갱신한다(실패에도 갱신되면 거짓말이 된다). `history.jsonl` 에는 실패도 남는다

**실측(2026-08-28):** 전량 3분 → **증분 50초**(변경 10,545개) → **재실행 14초**(무변경). 실패 경로 넷을 시험했다 — 같은 디스크·없는 원본·`--keep 0`·거부된 목적지의 부작용. 세대 정리는 격리 시험으로 확인했다(형식이 맞는 오래된 것만 지운다).

> ⚠ **작업 스케줄러에 등록하지 않았다.** 리더 PC 에 상주하는 것을 만드는 건 결재 사안이라 등록 명령만 넘겼다. **등록 전까지 이건 "손으로 돌리는 스크립트"이고, 안 돌리면 백업은 여전히 1회짜리다.**

### 7-8. 남은 것

- **오프사이트가 없다.** 이 집에 불이 나면 F:·H: 둘 다 없다 → §7-5 의 R2. 아직 안 했다
- **자동 실행이 없다** — 위 ⚠
- **복원을 실제로 해본 적은 없다.** §7-6 ③의 절차는 **읽어서 옳지 실행으로 검증된 게 아니다.** §9 의 새 기계 완주가 그 검증이다 — 거기서 이 절차가 부트스트랩 6단계가 된다
