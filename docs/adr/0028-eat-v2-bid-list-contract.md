...
## ⚠ 선행 미해결 — 이 ADR 을 Accepted 로 올리기 전에

```
🔴 parquet_v2(레거시 레이크)가 원시의 22%(경남 4.4%)만 적재된 원인이 미상이다
   중단인가 · 파서 결함인가 · 의도된 필터인가 — **셋이 처방이 다르다**
⟹ 결함이거나 필터면 같은 구멍이 eat-v2 에서도 날 수 있다.  원인을 먼저 밝힌다
```
