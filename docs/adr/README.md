...
| [0057](0057-duplicate-eligibility-area-code-is-one-area.md) | Accepted | 참가제한지역에 같은 `PDLC_CD`가 두 번 오면 같은 지역을 두 번 말한 것이다 — 먼저 온 줄을 남기고 합친 사실은 실행 로그에 남긴다. 판정은 코드 하나이고 라벨은 다듬지 않는다 |
