...
**party 승격 규칙.** `eat:business-number` 관측이 있으면 그 code value가 party의 유일 키이고, 같은
...
레이크 전수에서 개찰 시각 결측이 몇 건인지는 별도 조사로 세어 이 절에 숫자로 남긴다. **다만 결측이
0건으로 나와도 `not null`로 좁히지 않는다.** 미래 관측 한 건의 결측이 발행 전체를 격리시키는 위험이
