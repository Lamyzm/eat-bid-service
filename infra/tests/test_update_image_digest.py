...
def test_지정한_image_digest만_교체하고_나머지_byte는_보존한다(
    tmp_path: Path,
) -> None:
...
def test_semantic_또는_decorated_top_level_images_key를_변경하지_않고_거부한다(
    tmp_path: Path,
...
def test_multiple_YAML_document를_변경하지_않고_거부한다(tmp_path: Path) -> None:
    manifest = tmp_path / "kustomization.yaml"
    _write_manifest(
        manifest,
        _yaml(
...
def test_non_plain_top_level_grammar를_변경하지_않고_거부한다(
    tmp_path: Path,
    ambiguous_top_level: str,
) -> None:
    manifest = tmp_path / "kustomization.yaml"
...
def test_중복_unrelated_top_level_key를_변경하지_않고_거부한다(
    tmp_path: Path,
) -> None:
    manifest = tmp_path / "kustomization.yaml"
    _write_manifest(
...
def test_target_변경_전에_모든_image_entry를_검증한다(
    tmp_path: Path,
    foreign_entry: str,
) -> None:
    manifest = tmp_path / "kustomization.yaml"
...
def test_non_string_image_name을_domain_error로_거부한다(
    tmp_path: Path,
...
def test_non_string_digest를_domain_error로_거부한다(
    tmp_path: Path,
    invalid_value: object,
) -> None:
    from infra.update_image_digest import DigestUpdateError, update_digest
...
def test_유효하지_않은_path를_domain_error로_거부한다(invalid_path: object) -> None:
    from infra.update_image_digest import DigestUpdateError, update_digest
...
def test_유효하지_않거나_ambiguous한_input을_변경하지_않고_거부한다(
    tmp_path: Path,
    manifest_text: str,
    image_name: str,
    digest: str,
...
def test_CLI는_거부한_update를_변경하지_않고_exit_2를_반환한다(
    tmp_path: Path,
) -> None:
    manifest = tmp_path / "kustomization.yaml"
    before = _write_manifest(
